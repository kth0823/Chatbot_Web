...
App.Face={init:function(){$(".list_photo .link_thumb").on("click",App.Face.onclick_sample)},onclick_sample:function(e){$(".list_photo .on").removeClass("on");
	...
	if(image_url){fd.append("image_url",image_url)}else{fd.append("image",image_file)}$.post(
			{
				...
				contentType:false,
				timeout:1e4,beforeSend:function(){}
			}).done(
					function(data){
						...
						face_result=result;
						App.Face.apply_face_result(face_result)}).fail(function(xhr,error,text){App.show_request_error(xhr,xhr.responseText,"#face_result")}).always(function(){App.show_preloader(false)})},apply_face_result:function(resp){$("#face_result").html(JSON.stringify(resp,null,4));
						$("#face_sample_image_box .face_box_wrapper").remove();
	...
	var face_mask_canvas=$("#face_mask_canvas").show();
	face_mask_canvas.css({left:image_left,top:image_top,width:scaled_width,height:scaled_height});
	face_mask_canvas[0].width=scaled_width;face_mask_canvas[0].height=scaled_height;
	var face_mask_ctx=face_mask_canvas[0].getContext("2d");
	face_mask_ctx.fillStyle="rgba(0,0,0,0.27)";
	...
	var rect=$("<div class='face_rect'><span class='info'>"+info+"</span></div>"); // 나이성별 정보
	...
	var ctx_scale_x=100/face.w;
	var ctx_scale_y=100/face.h;
	for(var region in face.facial_points){
		var points=face.facial_points[region];
		for(var k=0;k<points.length;k++){
			ctx.beginPath();
	ctx.ellipse((points[k][0]-face.x)*ctx_scale_x,(points[k][1]-face.y)*ctx_scale_y,1,1,0,0,2*Math.PI);
	ctx.fill();
	ctx.closePath()}}canvas=$("<canvas class='facial_angle' width='400' height='400' style='position:absolute;'></canvas>");
	...
	App.Product={detect:function(image_file,image_url){App.show_preloader(true);$("#product_sample_image_box .product_box_wrapper").remove();
	var fd=new FormData;
	if(image_url){fd.append("image_url",image_url)}else{fd.append("image",image_file)}$.post({url:"api/product",data:fd,processData:false,contentType:false,timeout:1e4,beforeSend:function(){}}).done(function(data){var result=JSON.parse(data);product_result=result;
	App.Product.apply_product_result(product_result)}).fail(function(xhr,error,text){App.show_request_error(xhr,xhr.responseText,"#product_result")}).always(function(){App.show_preloader(false)})},apply_product_result:function(resp){$("#product_result").html(JSON.stringify(resp,null,4));$("#product_sample_image_box .product_box_wrapper").remove();var prduct_sample_image=$("#product_sample_image_box");
	var image_width=resp.result.width;
	var image_height=resp.result.height;
	var box_width=prduct_sample_image.width();
	var box_height=prduct_sample_image.height();
	var x_scale=box_width/image_width;
	var y_scale=box_height/image_height;
	var scale=Math.max(x_scale,y_scale);
	var scaled_width=image_width*scale;
	...
	var image_left=-(scaled_width-box_width)/2;
	var image_top=-(scaled_height-box_height)/2;
	var product_mask_canvas=$("#product_mask_canvas").show();
	product_mask_canvas.css({left:image_left,top:image_top,width:scaled_width,height:scaled_height});
	product_mask_canvas[0].width=scaled_width;product_mask_canvas[0].height=scaled_height;
	var ctx=product_mask_canvas[0].getContext("2d");ctx.fillStyle="rgba(0,0,0,0.27)";
	ctx.fillRect(0,0,scaled_width,scaled_height);
	var result_box_wrapper=$("<div class='product_box_wrapper result_box_wrapper'></div>");
	result_box_wrapper.css({left:image_left,top:image_top,width:scaled_width,height:scaled_height});
	$("#product_sample_image_box").append(result_box_wrapper);
	for(var i=0;i<resp.result.objects.length;i++){var obj=resp.result.objects[i];
	var info=obj["class"];
	var x=obj.x1*100;
	var y=obj.y1*100;
	var w=(obj.x2-obj.x1)*100;
	var h=(obj.y2-obj.y1)*100;
	var info_class=x>50?"info":"info info_right";
	var rect=$("<div class='product_rect'><span class='"+info_class+"'>"+info+"</span></div>"); // info 변수 : 나이/성별 정보
	...
	var recog_word= $('#recog_word').val();
	//var resps=resp.result;
	var regExp = /[\{\}\[\]\/?.,;:ø|\)*~`!^\-+ø<>@\#$%&\\\=\(\'\"]/gi;
	var recog="";
	for (var i=0;i<resp.result.length;i++){
		console.log(resp.result[i].recognition_words); // 인식된 문자만 출력하는 로그창 작성
		recog+=resp.result[i].recognition_words;		
	}
	//var recog_word=recog.replace(/ /g,"");
	var calib=recog.replace(/ /g,"");	
	var region1;
	var region_no;
	var bus_type;
	var car_no;
	//
	var change;
	//change=calib.replaceAll(regExp,"");
	//recog_word= change.replaceAll("C","0"); // 임의 변경
	recog_word= calib.replaceAll("C","0"); // 임의 변경
	change= recog_word.replaceAll(regExp,""); // 임의 변경
	recog_word=change;
	console.log(recog_word);
	console.log(recog_word.indexOf(regExp));
	console.log(recog_word.length);
	/*var index=recog_word.indexOf(regExp, 0);
	if(regExp.test(recog_word)){
		if(index==recog_word.length){	 
			change=recog_word.slice(0, recog_word.length-1);
			recog_word=change;
		}
	}
	*/
	if(recog_word.length==8){
		region1=recog_word.slice(0, 1);
		console.log(region1);
		
		region_no=recog_word.slice(1, 3);
		console.log(region_no);
		
		bus_type=recog_word.slice(3, 4);
		console.log(bus_type);
	}
	
	if(recog_word.length==9){
		region1=recog_word.slice(0, 2);
		console.log(region1);
		
		region_no=recog_word.slice(2, 4);
		console.log(region_no);
		
		bus_type=recog_word.slice(4, 5);
		console.log(bus_type);

	}
	
	if(recog_word.length==10){
		change=recog_word.replaceAll(regExp,"");
		calib=change;
		region1=recog_word.slice(0, 2);
		console.log(region1);
	...
	car_no=recog_word.slice(-4, recog_word.length);
	console.log(car_no);
	
	//var rec_rect="<div class='recognition_words'><li>인식된 문자 : " +recog+"</li></div>"// 나이성별 정보	
	//$(".option_api").append(rec_rect);
	$('#recog_word').val(recog_word);
	$('#region_no').val(region_no);	
	$('#'+bus_type).prop("selected", true);
	$('#car_no').val(car_no);	
	$('#'+region1).prop("selected", true);
	var region_code;
	region_code=$('#car_regionid').val();
	var bus_typecode;
	bus_typecode=$('#bus_typeid').val();
	var car_code="306 ";	
	car_code="306 ";
	car_code+=region_code;
	car_code+=" ";
	car_code+=bus_typecode;
	car_code+=" ";
	car_code+=region_no;
	car_code+=car_no;
	$('#qr_be').val(car_code);	
	...
	$("#recog_word").on("change", function(){
		recog_word=$('#recog_word').val();
		region_code=$('#car_regionid').val();
		region_no=recog_word.slice(2, 4);
		bus_typecode=$('#bus_typeid').val();
		car_no=recog_word.slice(-4, recog_word.length);
	...
	$("#car_regionid").on("change", function(){
		recog_word=$('#recog_word').val();
		region_code=$('#car_regionid').val();
		region_no=recog_word.slice(2, 4);
		bus_typecode=$('#bus_typeid').val();
	...
	//var event=document.createEvent("HTML Events");
	//event.InitEvent("click",true,false);
	//document.getElentById("searchBtn").dispatchEvent(event);
	
	//document.searchBtn.click();
	//$('#searchBtn').prop("onclick", true);;
	...
	//openWin = window.open(content, "childForm", "width=570, height=350, resizable = no, scrollbars = no");
	/*$("#recog_word").on("propertychange change keyup paste input", function() {
...
//	$('#'+Co_id).prop("selected", true);
	//$('#car_regionid').val("8").prop("selected", true);
/*
	console.log($(car_region.car_region));
	for(var i=0;i<car_region.car_regionid;i++)
		if(car_region.car_region==region1){
			$('#car_regionid').val(car_region.car_regionid).prop("selected", true);
	} */
	//$('#recog_word').attr(recog);
	//document.getElementById("searchBtn").click();
	

	
	content="/board/car_no?&searchType=s&keyword=" + encodeURIComponent(car_no);    
	var redirectWindow = window.open(content,  "childForm", "width=570, height=350, resizable = no, scrollbars = no");
    $.ajax({
        type: 'GET',
        url: content,
	...
	img.onload=function(){sample_image.attr("org_width",this.width);sample_image.attr("org_height",this.height)}};reader.readAsDataURL(file)}});
	...
	App.run_api(this.section,null,url)},load_image:function(){
		var sample_image=$("#"+this.section+"_sample_image");
	var reader=new FileReader;
	reader.onload=function(e){
		sample_image[0].style.backgroundImage="url("+e.target.result+")"};
