	...
	<div id="root">
		...
		<section id="container">
			<form role="form" method="get" >
					...
					<c:forEach items="${FWlist}" var="FWlist">
					<tbody>
							<tr>
							...
							<!--  	<c:forEach var="user" items="${user}" varStatus="i">
						  		...
						  		<td><c:out value="${FWlist.fregdate}" /></td>								
								<td><c:out value="${FWlist.fw}" /></td>
								<td>								
								 <a href="/board/FWreadView?fno=${FWlist.fno}"><c:out value="${FWlist.fcomment}" /></a> 													
								</td>
							</tr>
					</tbody>
					</c:forEach>
					...
					<script>
				      ...
				      $(function(){
				        $('#searchBtn').click(function() {
				          self.location = "FWlist" + '${pageMaker.makeQuery(1)}' + "&searchType=" + $("select option:selected").val() + "&keyword=" + encodeURIComponent($('#keywordInput').val());
				        });
				      });   
				    </script>
				  </div>
				<div class="col-md-offset-3">
						...
						<c:if test="${pageMaker.prev}">
							<li><a
								href="FWlist${pageMaker.makeSearch(pageMaker.startPage - 1)}">이전</a></li>
						</c:if>

							<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
								<li <c:out value="${pageMaker.cri.page == idx ? 'class=info' : ''}" />>
								<a href="FWlist${pageMaker.makeSearch(idx)}">${idx}</a></li>
							</c:forEach>
						<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
							<li><a
								href="FWlist${pageMaker.makeSearch(pageMaker.endPage + 1)}">다음</a></li>
						</c:if>
					</ul>
				</div>								
			</form>
