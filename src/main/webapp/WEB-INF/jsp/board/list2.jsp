			...
			<form role="form" method="get" >
					...
					<tbody>
							...
							<!--  	<c:forEach var="user" items="${user}" varStatus="i">
								...
								<td>
									<c:out value="${list2.car_no}" />
								</td>
								<td>								
								<!--  <a href="/board/readView?bno=${list.bno}"><c:out value="${list.csr_reqid}" /></a> -->													
									<c:forEach var="csr_req" items="${csr_req}" varStatus="i">
										<a href="/board/readView?bno=${list2.bno}" target="_blank"><c:if test="${list2.csr_reqid eq csr_req.csr_reqid}">${csr_req.csr_req}</c:if></a>
									</c:forEach>												
								</td>
							</tr>
					</tbody>
		  		...
		  		<div class="search row">
					<div class="col-xs-2 col-sm-2">
				  	<select name="searchType1" class="form-control">
				      <option value="c"<c:out value="${scri.searchType1 eq 'c' ? 'selected' : ''}"/>>고속사</option>	      		
				    </select>
				  </div>
					<div class="col-xs-10 col-sm-10">
						<div class="input-group">
							<input type="text" name="keyword1" id="keywordInput1"
								value="${scri.keyword1}"class="form-control"/>
						</div>
					</div>
				  </div>
				  <div class="col-xs-2 col-sm-2">
				  <select name="searchType2" id="search2" class="form-control">
				      <option value="s"<c:out value="${scri.searchType2 eq 's' ? 'selected' : ''}"/>>차량번호</option>
		   	    </select>
				  </div>
				  <div class="col-xs-10 col-sm-10">
						<div class="input-group">
							<input type="text" name="keyword2" id="keywordInput2"
								value="${scri.keyword2}"class="form-control"/>
					...
					<script>
				      ...
				      $(function(){
				        $('#searchBtn').click(function() {
				          self.location = "list2" + '${pageMaker.makeQuery(1)}' + "&searchType1=" + $("select option:selected").val() + "&keyword1=" + encodeURIComponent($('#keywordInput1').val())+ "&searchType2=" + $("#search2 option:selected").val() + "&keyword2=" + encodeURIComponent($('#keywordInput2').val());
				        });
				      });   
				    </script>
				  </div> 
				<div class="col-md-offset-3">
						...
						<c:if test="${pageMaker.prev}">
							...
							<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
								...
								<a href="list2${pageMaker.makeSearch2(idx)}">${idx}</a></li>
							</c:forEach>
						<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
							<li><a
								href="list2${pageMaker.makeSearch2(pageMaker.endPage + 1)}">다음</a></li>
						</c:if>
					</ul>
				</div>
